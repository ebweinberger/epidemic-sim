...
INITIAL_SICK_PERCENTAGE = 0.0001
...
#There is a chance that the person will move up, right, down, or left if the space is available
def personMove(i, j):
    ...
    if direction>=.50 and direction<.75:
        if ((i+1)<len(environment)):
            if environment[i+1][j] == None:
                ...
                environment[i][j] = None
    #left
    if direction>=.78 and direction<=1:
        if((j-1)>0):
            if environment[i][j-1] == None:
                environment[i][j-1] = environment[i][j]
                environment[i][j] = None
